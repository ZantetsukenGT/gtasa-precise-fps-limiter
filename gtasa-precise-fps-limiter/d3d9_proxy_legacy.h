...
class d3d9_device_proxy_legacy : public IDirect3DDevice9
{
public:
	// aca vamos a pedir un ptr el cual apuntara al device q el juego usa originalmente
	d3d9_device_proxy_legacy(IDirect3DDevice9& dev, s64 targetFramerate)
		: m_pDirect3DDevice9{ dev }
		, frameLimiter(targetFramerate) {}
	virtual ~d3d9_device_proxy_legacy() {}
	...
	STDMETHOD_(UINT, GetAvailableTextureMem)(void);
	STDMETHOD(EvictManagedResources)(void);
	...
	STDMETHOD_(UINT, GetNumberOfSwapChains)(void);
...
private:
	...
	LegacyFrameLimiter frameLimiter;
};
